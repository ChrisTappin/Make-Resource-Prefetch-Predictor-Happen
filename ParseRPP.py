...
csv_help_text = """Enable CSV mode (disabled by default).\n\
  ...
  Disabled: Prints the host and converted timestamp then lists each origin. Skips less interesting redirects."""
...
parser.add_argument("-p", "--prefix", default=" -", help="Add text before each origin is printed, or set to "" for none. Defaults to a single space followed by a dash.")
parser.add_argument("-s", "--suffix", default="", help="Add textafter each origin is printed. Defaults to none.")
...
parser.add_argument("-o", "--output", default=datetime.datetime.now(datetime.timezone.utc).strftime('%Y-%m-%d_%H%M%S.txt'), action="store", \
                    help="Output file name to write if CSV mode is enabled. Defaults to timestamp in format YYYY-MM-DD_HHMMSS with .txt extension.")
...
def ListRPPO(RPPOblob):
    if csv_mode == True:
        with open(args.output, 'a', encoding="utf-8") as csv:
